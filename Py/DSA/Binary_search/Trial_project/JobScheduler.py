...
def get_job_input_details():
    ...
    while True:
        try: 
            ...
            print("Please enter a number for number in minutes")
            duration_of_job = input("Enter the duration of the job in minutes, ex 60-> ")
        else:
            break
...
while True:
    ...
    if int(selection) == 1:
        ...
        if result:
            if result.name_of_job == job_name and result.duration_of_job == duration_of_job and result.start_time == start_time:
                print("Removing job")
                print(result)
